...
if __name__ == "__main__":
    # First connect/open/read/loads to the data url users
    url_first = get('https://jsonplaceholder.typicode.com/todos/')
    data_url_first = url_first.json()
    completed = 0
    TOTAL_NUMBER_OF_TASKS = 0
    NUMBER_OF_DONE_TASKS = []
    # Second connect/open/read/loads to the data url todos
    ...
    for i in data_url_second:
        if i.get('id') == int(argv[1]):
            employee = i.get('name')
    # Third connect/open/read/loads to the data url todos
